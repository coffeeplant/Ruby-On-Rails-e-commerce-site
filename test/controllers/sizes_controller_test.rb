...
class SizesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create size" do
    assert_difference('Size.count') do
      post sizes_url, params: { size: { calc: @size.calc, description: @size.description, size: @size.size } }
    end
  ...
  test "should update size" do
    patch size_url(@size), params: { size: { calc: @size.calc, description: @size.description, size: @size.size } }
